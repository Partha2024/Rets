...
import { MuscleRegion } from './components/muscle-heatmap/muscle-heatmap.component';
...
const BODYWEIGHT_LOAD_COEFFICIENT = 0.5;
const SECONDS_PER_EQUIVALENT_REP = 3;
...
export class WorkoutSessionDetailsPage implements OnInit {
  ...
  muscleHeatmap: Record<MuscleRegion, number> = this.emptyHeatmap();
  summary: SummaryView = {
    totalVolume: 0,
    totalSets: 0,
    totalReps: 0,
  ...
  //       this.muscleHeatmap[region] += volume
  //     }
  ...
  private buildViewModel(session: workoutSession) {
    ...
    let totalTimeSeconds = 0;
    let topSet: TopSetView | undefined;
    this.muscleHeatmap = this.emptyHeatmap();

    session.exerciseLogs.forEach((log: ExerciseLog, index: number) => {
      const exerciseId = log.exerciseId?.toString() || 'unknown';
      const exerciseInfo = this.exerciseLookup.get(exerciseId);
      console.log('exerciseId:', exerciseId, 'found:', exerciseInfo, 'muscleGroup:', exerciseInfo?.muscleGroup, 'primaryMuscle:', exerciseInfo?.primaryMuscle);
      ...
      // Real, externally-loaded volume — used for all user-facing totals.
      const displayVolume = reps > 0 && hasWeight ? reps * weight : 0;

      // Estimated volume — drives the heatmap only, so bodyweight/timed
      // work still registers instead of leaving the region blank.
      const heatmapVolume = this.estimateHeatmapVolume(reps, weight, timeSeconds);
      ...
      group.totalSets += 1;
      group.totalReps += reps;
      group.totalVolume += displayVolume;
      group.bestWeight = Math.max(group.bestWeight, weight);
      ...
      if (region && heatmapVolume > 0) {
        this.muscleHeatmap[region] += heatmapVolume;
      }
      ...
      const primary = hasTimed ? this.formatDuration(timeSeconds) : `${reps} reps`;
      const secondary = hasTimed ? 'Timed' : hasWeight ? `${weight} kg` : 'Bodyweight';
      const volumeLabel = displayVolume > 0 ? `${Math.round(displayVolume)} kg` : '-';
  ...
  private estimateHeatmapVolume(reps: number, weight: number, timeSeconds: number): number {
    if (weight > 0 && reps > 0) {
      return reps * weight;
    }
    if (timeSeconds > 0) {
      const equivalentReps = timeSeconds / SECONDS_PER_EQUIVALENT_REP;
      return equivalentReps * ASSUMED_BODYWEIGHT_KG * BODYWEIGHT_LOAD_COEFFICIENT;
    }
    if (reps > 0) {
      return reps * ASSUMED_BODYWEIGHT_KG * BODYWEIGHT_LOAD_COEFFICIENT;
    }
    return 0;
  }

    private emptyHeatmap(): Record<MuscleRegion, number> {
    return {
      chest: 0,
      shoulders: 0,
      biceps: 0,
  ...
  private resolveMuscleRegion(muscleGroup: string, primaryMuscle: string): MuscleRegion | null {
    ...
    const fromPrimary = this.mapMuscleText(primary);
    if (fromPrimary) {
      return fromPrimary;
    }

    return this.mapMuscleText(group);
  }
  ...
  if (value.includes('back') || value.includes('lats') || value.includes('latissimus') || value.includes('rhomboid') || value.includes('trapezius')) {
    return 'back';
  }
  if (value.includes('shoulder') || value.includes('deltoid')) {
    return 'shoulders';
  }
  if (value.includes('forearm')  || value.includes('forearms') || value.includes('arms') || value.includes('arm')) {
    return 'forearms';
  }
  if (value.includes('bicep') || value.includes('biceps')) {
    return 'biceps';
  }
  if (value.includes('tricep') || value.includes('triceps')) {
    return 'triceps';
  }
  if (value.includes('core') || value.includes('ab') || value.includes('abdominal')) {
    return 'core';
  }
  if (value.includes('quad') || value.includes('quads')) {
    return 'quads';
  }
  if (value.includes('hamstring') || value.includes('hamstrings')) {
    return 'hamstrings';
  }
  if (value.includes('calf') || value.includes('calve') || value.includes('calves')) {
    return 'calves';
  }
  if (value.includes('leg') || value.includes('legs')) {
    return 'quads';
  }
  return null;
}
